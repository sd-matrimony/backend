...
function changed(raw: string | null | undefined, next: string): boolean {
  return raw === undefined || raw === null ? next !== '' : raw !== next
}

async function run() {
  const { state: statePath, apply, restart } = resolveArgs()
  const state = loadState<CheckpointState>(statePath, restart, {
    lastId: null,
    scanned: 0,
    changed: 0,
  })
  ...
  if (state.lastId) console.log(`Resuming after _id=${state.lastId} (scanned=${state.scanned}, changed=${state.changed})`)
  ...
  while (true) {
    const query: Record<string, unknown> = state.lastId
    ...
    for (const u of batch as any[]) {
      ...
      if (!casteChanged && !subCasteChanged) continue
      ...
      if (casteChanged) set['otherDetails.caste'] = newCaste
      if (subCasteChanged) set['otherDetails.subCaste'] = newSubCaste

      ops.push({ updateOne: { filter: { _id: u._id }, update: { $set: set } } })
    }

    if (apply && undoLines.length) appendFileSync(undoPath as string, undoLines.join('\n') + '\n')
