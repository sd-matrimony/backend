...
function userSelectMap(prefix = "") {
    return userSelect.split(" ").reduce((prev, curr) => {
...
export async function getMarriedUsers(c) {
    const queries = c.req.valid("query") || { limit: 10, skip: 0 };
    const numLimit = Number(queries?.limit || 10);
    const numSkip = Number(queries?.skip || 0);
...
export async function createUsers(c) {
...
export async function userMarriedTo(c) {
