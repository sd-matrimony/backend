...
export const createOrder = async (c) => {
    ...
    const { subscribedTo = "basic", noOfProfilesCanView = 30, isAssisted = false, assistedMonths = 0 } = c.req.valid("json");
...
export async function onPaid({ _id, role }, body) {
    if (!body.isAssisted) {
        body.assistedMonths = 0;
    }
    const expiryDate = new Date(new Date().setMonth(new Date().getMonth() + planValidityMonths[body.subscribedTo]));
...
export const testCreateOrder = async (c) => {
    const { _id } = c.get("user");
...
export const testVerifyPayment = async (c) => {
