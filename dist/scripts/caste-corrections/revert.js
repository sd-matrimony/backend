...
async function run() {
    ...
    if (rows.length === 0)
        throw new Error(`Undo file is empty: ${undo}`);
    ...
    for (const row of rows) {
        ...
        await User.updateOne({ _id: new mongoose.Types.ObjectId(row.id) }, [{ $set: { 'otherDetails.caste': row.caste, 'otherDetails.subCaste': row.subCaste } }]);
