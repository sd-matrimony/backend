...
export function t(c, key) {
    return messages[getLocale(c)][key];
}
export function tf(c, key, vars) {
    let str = messages[getLocale(c)][key];
...
const staticTextToTa = Object.fromEntries(Object.entries(staticEn).map(([k, enText]) => [enText, staticTa[k]]));
export function translateStatic(c, enText) {
    if (getLocale(c) !== "ta")
        return enText;
